...
def multi_document_summary(search_results, query: str, limit=5):
    results = []

    for result in search_results[:limit]:
        results.append(f"{result['title']}: {result['document']}")
    prompt = f"""
...
def generate_answer_with_citations(search_results, query: str, limit=5):
    documents = []

    for result in search_results[:limit]:
        documents.append(f"{result['title']}: {result['document']}")
...
def answer_question(search_results, question: str, limit=5):
    documents = []
...
def summarize(query: str, limit=DEFAULT_SEARCH_LIMIT):
    ...
    answer = multi_document_summary(search_results, query, limit)
...
def generate_citations(query: str, limit=DEFAULT_SEARCH_LIMIT):
    movies = load_movies()
    hybrid_search = HybridSearch(movies)
...
def citations_command(query: str, limit: int = DEFAULT_SEARCH_LIMIT):
    return generate_citations(query, limit)
...
def question_command(question: str, limit: int = DEFAULT_SEARCH_LIMIT):
    ...
    answer = answer_question(search_results, question, limit)
